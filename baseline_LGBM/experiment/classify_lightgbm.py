...
def classify():
    ...
    # Filter for patients that have a path_severity label
    df_clinical = df_clinical[df_clinical['path_severity'].notna()]
    # Convert path_severity to numeric
    df_clinical['path_severity'] = pd.to_numeric(df_clinical['path_severity'], errors='coerce')
    df_clinical = df_clinical[df_clinical['path_severity'].notna()]
    
    # Get max severity per patient (assuming higher severity is the primary finding)
    df_labels = df_clinical.groupby('empi_anon')['path_severity'].max().reset_index()
    ...
    # LightGBM is generally faster and handles large feature sets well
