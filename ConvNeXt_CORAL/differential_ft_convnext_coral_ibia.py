...
def train_full_model(model, train_loader, val_loader, device, epochs=50):
    # Unfreeze everything with differential LRs
    for param in model.parameters():
        param.requires_grad = True

    optimizer = optim.Adam([
        {'params': model.backbone.parameters(), 'lr': 1e-5},  # slow backbone
        {'params': model.fc.parameters(),       'lr': 1e-3},  # fast head
...
def main():
    ...
    print("Starting differential fine-tuning (Backbone 1e-5, Head 1e-3)...")
    model = train_full_model(model, train_loader, val_loader, device, epochs=50)
    ...
    print(f"Head recalibration Kappa (ConvNeXt): see recalibration results")

    output = {
        'differential_finetuning_convnext_coral': {
    ...
    out_path = '/home/tanuh/Desktop/TANUH_Keerthana/Mammography_Datasets/mammography-datasets/ibia_convnext_differential_ft_results.json'
