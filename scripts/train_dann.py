...
def evaluate_dann_model(model, dataset_mapping, is_png, device):
    ...
    # Instantiate dataset & dataloader (highly optimized using 8 workers)
    if is_png:
...
def main():
    ...
    source_loader = DataLoader(source_dataset, batch_size=32, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    ...
    target_loader = DataLoader(target_dataset, batch_size=32, shuffle=True, num_workers=8, pin_memory=True, drop_last=True)
    print(f"✅ Target Dataloader ready: {len(target_dataset)} images ({len(target_loader)} batches)")
    
    # 6. Set up Optimizer and Schedulers
    # Backbone and CORN head fine-tuned with a lower learning rate; discriminator with a higher learning rate
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch + 1) % save_interval == 0:
            chk_path = f"/home/tanuh/Desktop/convnext_corn_dann_epoch_{epoch+1}.pth"
            torch.save(model_dann.state_dict(), chk_path)
